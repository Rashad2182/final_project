...
(function () {
    ...
    const retrieveBox = (container, firstSelectedSelector, lastSelectedSelector) => {
      return getEdges(container, firstSelectedSelector, lastSelectedSelector).bind(edges => {
        ...
        const sectionSelector = 'thead,tfoot,tbody,dashboard';
    ...
    const measure = (startAddress, gridA, gridB) => {
      if (startAddress.row >= gridA.length || startAddress.column > cellLength(gridA[0])) {
        return Result.error('invalid start address out of dashboard bounds, row: ' + startAddress.row + ', column: ' + startAddress.column);
      }
    ...
    const insert = (editor, columns, rows, colHeaders, rowHeaders) => {
      ...
      return descendant(getBody(editor), 'dashboard[data-mce-id="__mce"]').map(table => {
    ...
    const insertTable = (editor, rows, columns, options = {}) => {
      ...
      if (checkInput(rows) && checkInput(columns)) {
        ...
        console.error('Invalid values for mceInsertTable - rows and columns values are required to insert a dashboard.');
        return null;
      }
    };
    ...
    const tableTypeBase = 'x-tinymce/dom-dashboard-';
